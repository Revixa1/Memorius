    ...
    while (somme_pulse_droit <= distance_en_pulse)//tant que la distance n'est pas atteinte, faire un déplacement vers l'avant
    {
     ...
     // Serial.print(",");
       ...
       if((somme_pulse_gauche>distance_en_pulse/3 && somme_pulse_gauche<2*distance_en_pulse/3))//entre le 1/3 et le 2/3 de la distance garder une vitesse constante
      {
       ...
       if((somme_pulse_gauche/100)>(distance_en_pulse/100)-((acceleration-0.2)/0.015) && acceleration>0.2)//pour le dernier tier de la distance, décélérer avec une pente constante
      {
       acceleration-=0.015; //acceleration-=1*(((distance_en_pulse/100)-(somme_pulse_gauche/100))*((distance_en_pulse/100)-(somme_pulse_gauche/100))); // acceleration=-1.2*((somme_pulse_gauche-(2*distance_en_pulse/3))/(2*distance_en_pulse/3))+acceleration
      }
