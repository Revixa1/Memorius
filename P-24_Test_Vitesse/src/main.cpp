...
void loop()
{
  ...
  {                            // si la limit switch arrère est pesé, on rentre dans le if
   ...
   // deplacement(0,-1440);
    ...
    deplacement(180, 47); // déplacement de 40.4° (40.4° vers la gauche) en rotation et de 180 cm vers l'avant
    deplacement(60, -90);
    deplacement(80, 47);
    deplacement(80, -180);
    deplacement(60, -47);
...
{ // déclaration de la fonction de déplacement
  ...
  float KP = 0.000000000000001;       //Valeur pour tunner le gain proportionnelle du PID
  float KI = 0.001;           //Valeur pour tunner le gain de l'intégrale du PID
  ...
  KP = 0.00001; //Valeur pour tunner le gain proportionnelle du PID
  ...
  if (distance != 0) //si une distance est spécifié faire un mouvement en ligne droite de cette distance
  {
    ...
    while (somme_pulse_gauche <= distance_en_pulse)//tant que la distance n'est pas atteinte, faire un déplacement vers l'avant
    {
      ...
      while(100>ENCODER_Read(1)){}//on attend au moins 100 pulses avant de passer dans le PID
     ...
     // Serial.print(",");
       ...
       if((somme_pulse_gauche>distance_en_pulse/3 && somme_pulse_gauche<2*distance_en_pulse/3))//entre le 1/3 et le 2/3 de la distance garder une vitesse constante
      {
       ...
       if((somme_pulse_gauche/100)>(distance_en_pulse/100)-((acceleration-0.2)/0.05) && acceleration>0.15)//pour le dernier tier de la distance, décélérer avec une pente constante
      {
       acceleration-=0.05; //acceleration-=1*(((distance_en_pulse/100)-(somme_pulse_gauche/100))*((distance_en_pulse/100)-(somme_pulse_gauche/100))); // acceleration=-1.2*((somme_pulse_gauche-(2*distance_en_pulse/3))/(2*distance_en_pulse/3))+acceleration
      }
      /*
      Serial.print("somme_pulse_gauche ");
      Serial.print(somme_pulse_gauche);
